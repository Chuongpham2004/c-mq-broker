...
typedef enum {
    ...
    MSG_ACK = 4,       // Server phản hồi thành công hoặc Client ACK tin nhắn
    MSG_UNSUBSCRIBE = 5 // Client hủy đăng ký nhận tin từ Topic
} MessageType;

// Cấu trúc Header cố định (Tổng cộng 14 bytes)
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t msg_id;        // ID của tin nhắn (4 bytes)
    uint32_t timestamp;     // Nhãn thời gian epoch (4 bytes)
